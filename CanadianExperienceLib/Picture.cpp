...
void Picture::Save(const wxString& filename)
{
    ...
    //
    root->AddAttribute(L"MachineOneNum",wxString::FromDouble(mMachine->GetMachineOneNum()));
    root->AddAttribute(L"MachineTwoNum",wxString::FromDouble(mMachine->GetMachineTwoNum()));

    root->AddAttribute(L"MachineOneFrame", wxString::FromDouble(mMachine->GetStartFrame1()));
    root->AddAttribute(L"MachineTwoFrame", wxString::FromDouble(mMachine->GetStartFrame2()));
...
void Picture::Load(const wxString& filename)
{
    ...
    //
    mMachine->SetStartFrame1(wxAtoi(root->GetAttribute(L"MachineOneFrame", L"1")));
    mMachine->SetStartFrame2(wxAtoi(root->GetAttribute(L"MachineTwoFrame", L"1")));

    mMachine->SetMachineOneNum(wxAtoi(root->GetAttribute(L"MachineOneNum", L"1")));
    mMachine->SetMachineTwoNum(wxAtoi(root->GetAttribute(L"MachineTwoNum", L"2")));
